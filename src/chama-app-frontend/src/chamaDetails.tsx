...
import { FaArrowAltCircleRight, FaCalendarCheck, FaChartLine, FaCircle, FaCoins, FaCommentDots, FaDollarSign, FaInfoCircle, FaPen, FaPlus, FaRegWindowClose, FaUser } from "react-icons/fa"
...
export function ChamaDetails({currency="USD",tradeLimit=5000000000,setViewState,userName,chamaName,receiver,chamaMembers,daysRemaining,totalContributions,contributionAmount,nextPayoutDate}:{currency:string,tradeLimit:number,setViewState:Dispatch<SetStateAction<JSX.Element>>,userName:string,daysRemaining:number,totalContributions:number,nextPayoutDate:string,receiver:string,chamaName:string,chamaMembers:Array<any>,contributionAmount:number}){
    ...
    let LogoutHandler  = function(e:any){
        setLogout(<Logout LogoutProp={identityLogoutHandler} loggedIn={authenticated} slogout={setLogout} />)
    }
            ...
            <div className="w-full  h-24 flex justify-between items-center bg-black z-20 px-8 sticky top-0">
                ...
                <div className="w-auto h-auto">
                    <div className="w-auto flex gap-4 items-center">
                        ...
                        <h2 onClick={loginHandler}  className="text-xl font-bold hidden md:block text-slate-600 line-clamp-1" >{authenticated == true ? userNameState : "sign-in"}</h2>
            ...
            <div className="w-full h-auto flex flex-col gap-4 items-center">
                <div className="w-full h-auto flex wrap gap-4 justify-around">
                    ...
                    <div className="w-1/3 md:w-1/4 h-auto bg-blue-100 rounded-lg  text-blue-600 px-2 py-4 sm:px-3 sm:py-6 md:px-8 md:py-6 flex gap-3 justify-center items-center">
                        <span className="text-blue-600 font-bold" >trade Limit</span>
                        <span className="p-2 rounded-lg text-xl md:text-2xl bg-white text-pink-600 font-bold flex items-end gap-2"><span className="text-sm text-black" >{currency}</span>{valuesFormatter({amount:String(tradeLimit)})}</span>
                    </div>
                </div>
                <div className="w-full px-12 h-3/4  flex gap-12 justify-evenly md:justify-between items-center flex-wrap">
                    <div className="w-full md:w-1/2 justify-self-start h-auto flex flex-col rounded-lg p-4 md:px-8 bg-slate-400 ">
                        ...
                        <div className="w-full h-auto flex flex-col mb-4 rounded-lg bg-blue-100 gap-3 p-4 md:px-8 py-4 border-l-4 border-blue-600">
                            ...
                            <p className="w-full text-xl font-bold text-blue-600"><span className="text-sm text-slate-600 mr-1" >USD</span>{valuesFormatter({amount:String(expectedAmount)})}</p>
                        </div>
                        <div className="w-full h-auto flex flex-col mb-4 rounded-lg bg-blue-100 gap-3 p-4 md:px-8 py-4 border-l-4 border-blue-600">
...
export function Logout({LogoutProp,slogout,loggedIn}:{LogoutProp:(e:any) => void,slogout:Dispatch<SetStateAction<JSX.Element>>,loggedIn:boolean}){
    ...
    return(
        <div  className="w-auto h-auto  flex z-20 flex-col absolute top-12 right-1 gap-3 px-3 py-3 bg-slate-600 rounded-lg">
            ...
            <h3 className="flex gap-2 items-center font-bold cursor-pointer" onClick={logoutHandler}><FaArrowAltCircleRight className={clsx("text-xl",{"text-red-600":loggedIn == true,"text-blue-600":loggedIn == false})}/>{loggedIn == true ? "Logout" : "Login"} </h3>
        </div>
    )
}

export function RoundDetails({dueDate="22/7/2025",receiversRemaining=100,currentReceiver="Rogetz",payout=20000,currency="BTC",totalParticipants=2000,activeContributors=1600,roundNo=20,countDown="20:22:36:24",percentageContributors="80"}:{receiversRemaining ?: number,dueDate ?:string,payout ?:number,currency ?:string,currentReceiver ?:string,totalParticipants ?:number,activeContributors ?:number,percentageContributors ?:string,roundNo ?:number,countDown ?:string}){
        ...
        <div className="w-full h-full flex flex-col gap-3 px-4 py-4" >
            ...
            <div className="flex justify-between items-center border-b-2 border-blue-600">
                <div className="flex flex-col gap-2">
                    ...
                    <div className="font-bold" >{totalParticipants}</div>
                </div>
                <div className="flex flex-col gap-2">
...
export function ContributionStats({contributors=fakeContributors}:{contributors ?:Array<Contributor>}){
